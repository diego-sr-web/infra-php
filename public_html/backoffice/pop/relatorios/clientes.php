 ...
 * @param int|string $tipoProjeto
 * @param int|string $clientId
 *
 * @return array
 */
function generateMean($database, $tipoProjeto, $clientId) {
    $tempoMedioProjetos = [];
    $tempoMedioTarefas = [];
    $chatParams = [$tipoProjeto, $clientId];
    $sqlChat = "
            SELECT
...
if ($clientId != 0) {
    $cliente = $BNCliente->getDataWithId($clientId);
    $user["nome"] = $cliente["nomeFantasia"];
